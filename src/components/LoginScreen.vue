        ...
        <form @submit.prevent="handleLogin" class="space-y-5">
          <div>
            ...
            <div class="relative">
              ...
              <input v-model="bandName" type="text" id="bandName" required autocomplete="username"
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input v-model="bandKey" type="password" id="bandKey" required autocomplete="current-password"
                class="w-full pl-10 pr-4 py-3 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition text-base"
...
const handleLogin = async () => {
  ...
  try {
    ...
    // Verificar primero el campo 'password' específicamente (el más común)
    let passwordMatch = false
    let passwordField = null

    console.log('🔍 Verificando contraseña en campo "password"...')

    // Verificar primero el campo 'password' si existe
    if (bands.password !== undefined && bands.password !== null) {
      const storedPassword = String(bands.password).trim()
      const inputPassword = trimmedBandKey.trim()

      console.log('🔑 Comparando campo "password":', {
        ...
        coinciden: storedPassword === inputPassword,
        tipoAlmacenado: typeof bands.password
      })

      if (storedPassword === inputPassword) {
        passwordMatch = true
        passwordField = 'password'
        console.log('✅ ¡CONTRASEÑA CORRECTA! Campo usado: "password"')
      }
    } else {
      console.log('⚠️ Campo "password" no existe o es null/undefined')
      console.log('📋 Todos los campos disponibles:', Object.keys(bands))

      // Si no existe 'password', buscar en otros campos posibles
      ...
      console.log('🔍 Buscando en otros campos posibles:', allPossiblePasswordFields)

      for (const field of allPossiblePasswordFields) {
        const storedPassword = String(bands[field]).trim()
        const inputPassword = trimmedBandKey.trim()

        console.log(`🔑 Comparando campo "${field}":`, {
          valorAlmacenado: storedPassword.length > 0 ? storedPassword.substring(0, Math.min(10, storedPassword.length)) + (storedPassword.length > 10 ? '...' : '') : '(vacío)',
          longitudAlmacenada: storedPassword.length,
          valorIngresado: inputPassword.length > 0 ? inputPassword.substring(0, Math.min(10, inputPassword.length)) + (inputPassword.length > 10 ? '...' : '') : '(vacío)',
          longitudIngresada: inputPassword.length,
